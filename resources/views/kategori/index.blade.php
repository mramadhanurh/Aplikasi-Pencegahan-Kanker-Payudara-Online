    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" class="kategori form-control">
        </div>
      </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group mb-3">
            <label for="">Kategori</label>
            <input type="text" id="edit_kategori" class="kategori form-control">
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
<script>
    $(document).ready(function () {
        ...
        function fetchkategori()
        {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('tbody').html("");
                    var no = 1;
                    $.each(response.kategori, function (key, item) { 
                         $('tbody').append(
                            `<tr>
                                <td class="text-center">${no++}</td>
                                <td class="text-center">${item.kategori}</td>
                                <td class="text-center">
                                    <button class="btn btn-warning btn-sm btn-flat  edit_kategori" value="${item.id}"><i class="fas fa-pencil-alt"></i></button>
                                    <button class="btn btn-danger btn-sm btn-flat  delete_kategori" value="${item.id}"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>`
                         );
                    });
        ...
        $(document).on('click', '.delete_kategori', function (e) {
            ...
            // alert(usia_id);
            $('#delete_kategori_id').val(kategori_id);
        ...
        $(document).on('click', '.delete_data', function (e) {
            ...
            $(this).text("Deleting");
            var kategori_id = $('#delete_kategori_id').val();
            ...
            $.ajax({
                type: "DELETE",
                url: "/delete-kategori/"+kategori_id,
                success: function (response) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response.status == 404) {
                        ...
                        $('#edit_kategori').val(response.kategori.kategori);
        ...
        $(document).on('click', '.update_data', function (e) {
            ...
            $.ajax({
                type: "PUT",
                url: "update-kategori/"+kategori_id,
                data: data,
                dataType: "json",
                success: function (response) {
                    // console.log(response);
